...
public class SewooPrinter {
    ...
    private String logoImagePath = "C:\\OrderIt-Backend\\src\\main\\resources\\logo.bmp";
    @Async
    public void print(ReceiptDto receiptDto) {
        ...
        log.info("[SewooPrinter] port opening is successed");
        ...
        try {
            do {
                ...
                for (ReceiptDetailDto receiptDetailDto : receiptDetailDtoList) {
                    ...
                    log.info("[printDetails] {}", itemDetail);
            ...
            log.error("[printDetails] {}", ex.getMessage());
        }
    }
    ...
    private String formatLine(String name, int quantity, int price) {
        ...
        log.info("[formatLine] tempLength : {}", tempLength);
        if (tempLength > 27) {
            log.error("[formatLine] 28글자 넘음");
            temp.substring(0, 27);
        }
